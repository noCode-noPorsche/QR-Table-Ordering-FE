...
export function proxy(request: NextRequest) {
  ...
  if (refreshToken) {
    ...
    if (unAuthPaths.some((p) => pathname.startsWith(p)) && refreshToken) {
      if (loginPaths.some((p) => pathname.startsWith(p)) && searchParams.get('accessToken')) {
        return response
      }
    ...
    if (privatePaths.some((p) => pathname.startsWith(p)) && !accessToken && refreshToken) {
    ...
    if (isGuestGoToManagePath || isNotGuestToGuestPath || isNotOwnerToOwnerPath) {
      return NextResponse.redirect(new URL('/', request.nextUrl))
      // response.headers.set(
      //   "x-middleware-rewrite",
      //   new URL("/", request.nextUrl).toString(),
