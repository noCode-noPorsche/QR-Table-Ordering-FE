      ...
      <section className='bg-linear-to-r from-slate-900 via-slate-850 to-zinc-800 dark:from-zinc-950 dark:to-black rounded-2xl p-8 md:p-12 text-white border border-slate-800 shadow-xl relative overflow-hidden'>
        ...
        <div className='max-w-3xl space-y-4 relative z-10'>
          <div className='flex items-center gap-2 text-zinc-400'>
            ...
            <span className='text-xs font-semibold uppercase tracking-wider'>{t('contact.title')}</span>
          </div>
          <h2 className='text-2xl font-bold tracking-tight text-white'>{t('contact.description')}</h2>
          <p className='text-zinc-300 text-base md:text-lg leading-8 font-medium'>{t('contact.description')}</p>
        </div>
      </section>
    </div>
  )
